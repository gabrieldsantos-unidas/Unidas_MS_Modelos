    ...
    reader.onload = (e) => {
      try {
        ...
        const processedData: SalesForceCores[] = (jsonData as any[]).map((row: any) => ({
          // Id do registro IRIS_Produto_Cor__c (para update)
          ...
          ProductCode_Modelo: String(
            row['IRIS_Dispositvo__r.ProductCode'] || row['ProductCode_Modelo'] || ''
          ).trim(),
          IRIS_Dispositvo_Id: String(
            row['IRIS_Dispositvo__r.Id'] || row['IRIS_Dispositvo_Id'] || ''
          ).trim(),
          IRIS_Anodomodelo__c: String(
            row['IRIS_Dispositvo__r.IRIS_Anodomodelo__c'] || row['IRIS_Anodomodelo__c'] || ''
          ).trim(),

          IRIS_Cor_Name: String(row['IRIS_Cor__r.Name'] || row['IRIS_Cor_Name'] || '').trim(),
          IRIS_Cor_ID__c: String(
            row['IRIS_Cor__r.IRIS_Cor_ID__c'] || row['IRIS_Cor_ID__c'] || ''
          ).trim(),
          ProductCode_Cor: String(row['IRIS_Cor__r.ProductCode'] || row['ProductCode_Cor'] || '').trim(),

          // relacionamento IRIS_Cor__r.Id
          IRIS_Cor_RelatedId: String(row['IRIS_Cor__r.Id'] || row['IRIS_Cor_RelatedId'] || '').trim(),
